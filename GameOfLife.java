...
class Solution {
    ...
    public void gameOfLife(int[][] board) {
        ...
        for(int i = 0; i < m; i++){
            for(int j = 0; j < n; j++){
                // Count the live neighbors for each element
                int countLives = countLiveNeighbors(board, i, j);
                
                if(board[i][j] == 0){
                    // Marking it with a different number so that it doesn't interfere with the further computation
                    if(countLives == 3){
                        board[i][j] = 3;
                    }
                }
